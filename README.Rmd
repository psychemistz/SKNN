...
**Example**
You can download the package from [http://github.com/psychemistz/SKNN](http://github.com/psychemistz/SKNN)

<br>

1. Install
```{r SKNN}
install.packages("http://github.com/psychemistz/SKNN/blob/master/SKNN_1.0.0.tar.gz", repos=NULL, type="source")
```
...
2. Working Example
```{r SKNN-E}
data <- read.csv("https://raw.githubusercontent.com/psychemistz/SKNN/master/Example.csv", row.names = 1)
library(knitr)
kable(head(data[,1:5]))
dim(data)
sum(is.na(data))
...
```{r SKNN-c, echo=F}
library(knitr)
kable(read.csv("https://raw.githubusercontent.com/psychemistz/SKNN/master/Citation-Trend.csv"))
```
